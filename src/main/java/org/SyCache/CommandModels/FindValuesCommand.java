...
@CacheCommandAnnotation(commandName = "find-values",commandPattern = "REGEX TOP",commandDescription = "Find All Values That Match In Regex",singleCommand = "false")
public class FindValuesCommand extends CacheCommand {
    public FindValuesCommand(Cache cache) {
        super(cache);
    }
    ...
    public CacheResult execute(InterfaceBaseCommandModel command) throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException, ClassNotFoundException {
        ...
        Vector<Node> nodeVector=cache.getStorage().getPool(command.getPoolName()).findValues(command.getTailCommand()[0],Integer.parseInt(command.getTailCommand()[1]));
